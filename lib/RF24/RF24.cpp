...
void RF24::print_address_register(const char* name, uint8_t reg, uint8_t qty)
{
  ...
  {
    uint8_t buffer[5];
    uint8_t len = getAddressWidth();
    read_register(reg++,buffer,len);

    printf_P(PSTR(" 0x"));
    uint8_t* bufptr = buffer + len;
    while( --bufptr >= buffer )
...
void RF24::startListening(void)
{
  ...
  {
    write_register(RX_ADDR_P0, reinterpret_cast<const uint8_t*>(&pipe0_reading_address), getAddressWidth());
  }
...
void RF24::reUseTX(){
		write_register(RF_STATUS,_BV(MAX_RT) );			  //Clear max retry flag
...
bool RF24::txStandBy(){
	...
	while( ! (read_register(FIFO_STATUS) & _BV(TX_EMPTY)) ){
		if( get_status() & _BV(MAX_RT)){
			write_register(RF_STATUS,_BV(MAX_RT) );
			ce(LOW);
...
void RF24::read( void* buf, uint8_t len ){
  ...
  write_register(RF_STATUS,_BV(RX_DR) | _BV(MAX_RT) | _BV(TX_DS) );
...
void RF24::openWritingPipe(uint64_t value)
{
  ...
  uint8_t len = getAddressWidth();
  write_register(RX_ADDR_P0, reinterpret_cast<uint8_t*>(&value), len);
  write_register(TX_ADDR, reinterpret_cast<uint8_t*>(&value), len);
...
void RF24::openReadingPipe(uint8_t child, uint64_t address)
{
  ...
  if (child <= 6)
  {
    ...
    if ( child < 2 )
      write_register(pgm_read_byte(&child_pipe[child]), reinterpret_cast<const uint8_t*>(&address), getAddressWidth());
    else
...
/****************************************************************************/
void RF24::setAddressWidth( const uint8_t width )
{
  if (width < 2 || width > 5)
    return;
...
/****************************************************************************/
uint8_t RF24::getAddressWidth(void)
{
  return (read_register(SETUP_AW) & 0x03) + 2;
}
