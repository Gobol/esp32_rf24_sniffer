...
#define DEFAULT_COMPORT                 (8)
#define DEFAULT_RF_CHANNEL              (2)
#define DEFAULT_RF_DATARATE             (1)
...
#define DEFAULT_RF_BASE_ADDRESS         ((uint64_t)0xE7E7E7E7E7ULL)
...
{
  uint32_t channel;
  uint32_t rate;                        // rf24_datarate_e: 0 = 1Mb/s, 1 = 2Mb/s, 2 = 250Kb/s
  uint32_t addressLen;                  // Number of bytes used in address, range [2..5]
  uint32_t addressPromiscLen;           // Number of bytes used in promiscuous address, range [2..5]. E.g. addressLen=5, addressPromiscLen=4 => 1 byte unique identifier.
  uint64_t address;                    // Base address, LSB first.
  uint32_t crcLength;                   // Length of active CRC, range [0..2]
  uint32_t maxPayloadSize;              // Maximum size of payload for nRF (including nRF header), range[4?..32]
} serialConfig;
